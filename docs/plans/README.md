...
| Plan | 主题 | 备注 |
|---|---|---|
| [2026-08-29-p4c-context-runtime-graph-integration.md](2026-08-29-p4c-context-runtime-graph-integration.md) | P4c 实施：ContextRuntime 真正接入主图 + assembler 真实装 + golden before/after——4 graph caller 翻转到 `ContextRuntime.build()`（requirement_analysis + confirmed_execution + requirement_parser + sql_graph），主链 smoke 防 break，Selective Recall 8 用例收益矩阵 + legacy 对照钉，assembler 真实 Filter（dedup + §七 序）+ Token Budget 截断，golden before/after 验收 | 回链 [P3](2026-08-27-p3-context-runtime.md) + [P4a](2026-08-27-p4a-conversation-memory-decouple.md) + [P4b](2026-08-27-p4b-memory-lifecycle-selective-recall.md)；基于 [[p4c-start]] 用户两轮 review PASS 后 5 步优先级（graph caller 翻转 → 主链行为 → selective 收益 → assembler Filter/Budget → golden before/after）；master HEAD `efaeeb5` 已含 P3/P4a/P4b |
