...
| Plan | 主题 | 落地 |
|---|---|---|
| [2026-09-07-preference-explicit-wiring.md](2026-09-07-preference-explicit-wiring.md) | P16.5 Preference Explicit Wiring（简历盘点发现 + 用户拍板）：§五 契约 explicit→active 写入口无生产 caller——chat 入口图前短路（正则检测→remember_explicit_preference→idle 轻响应复用 chitchat 渲染，前端零改动）+ api 3 例 + 真 PG 闭环 1 例 | 分支 `p16-5-preference-explicit-wiring`：`76843d6`(plan) → `6826755`(接线+测试) → `91106ab`(专属用户修正)；全量 **1181 passed/1 skipped** 零回归；合 master `d1c10f4`；偏差 1 项（phase 变量复位 P11 同坑）见 plan 落地记录 |
| [2026-09-04-p16-memory-evaluation.md](2026-09-04-p16-memory-evaluation.md) | P16 Memory Evaluation & Behavioral Tests（Memory 封版）：report 链 ContextRuntime 偏好接线（§三 Report=Preference 契约补全，确定性 apply_chart_preference 通道，prompt/前端零改动）+ EXECUTION 档 drop preference + 8 行为测试（preference→chart ON/OFF 对照 / 不污染 SQL / query memory 参考帧 / candidate & expired & cross-user Agent 层）+ Layer 3 Gold Set（25 记忆 + 30 金标 queries 30/30×3 稳定 + 真 embedding runner Recall@1 0.84 / @3 0.96 / MRR 0.90 / Clean 1.0）+ G4 真 LLM SQL 门（已跑完：发现并修复参考帧年份诱导时间偏移——防御帧补「时间以 today 为准」）+ P16-LOW「表」宽 alias 已删 | 分支 `p16-memory-evaluation`：`5905998`(plan) → `719195c`(D4) → `2d94213`(preference 纯函数) → `81f5eec`(report 接线) → `073177d`(①⑧) → `106905e`(②⑤⑥⑦) → `532188d`(③④) → `f7e9ee5`(gold set) → `28b0c2d`(runner) → `50ce283`(回归适配+docs) → `c44a983`(G4 修复) → `cd74f29`(G4 记录) → `32285ec`(P16-LOW)；backend **1181 passed/1 skipped**（含 P16.5）+ evaluation 78 passed；合 master `c924db4`；偏差 3 项见 plan 落地记录 |
| [2026-09-03-final-hardening.md](2026-09-03-final-hardening.md) | Final Hardening（面试封版前收口，P0+P1 全收）：① schema 全链对齐 ②+⑥ SELECT INTO/行锁 + postgres dialect ③ Decimal 全链字符串化 ④ Argon2id ⑤ 确定性 object_not_found→MCP→repair replay ⑦ execute_sql 内建 EXPLAIN 门 ⑧ SQL 语义评估 ⑨ 并发 confirm ⑩ cross-user ⑪ 数据围栏注入防线 ⑫ CORS 白名单 ⑬ 最小 GitHub Actions ⑭ 文档/setup 对齐 ⑮ 最终回归 | master 15 commit（`bb2b197`→`1684598`）；backend **1116 passed/1 skipped**、frontend 301 passed、Contract **9/10 绿**（05=P13 已知红，per-PR 面排除）；抓到 P15 seam req=None 遗留回归 + Contract 端口泄漏假全红；关键发现与 live 手动门见 plan 落地记录 |
| [2026-09-03-p15-reliability-convergence.md](2026-09-03-p15-reliability-convergence.md) | P15 reliability 收口（用户 review `p15-e2e-live` 指令，不再扩业务 case）：① E2E gate `!= "1"` 统一（=0 不再误跑）② mcp_down ContextVar→create_task 继承契约 ③ MCP-down live seam 激活**日志 marker 直证**（弃空洞 requirement case：control 证明同 query 无 seam 也产 missing）④ `_patch_fill_all` 不吞错 ⑤ double_fact/empty_result 收紧 honest-terminal（LLM 2nd SQL 质量彩票，full-suite run#1 6/8 → run#2 **8/8**）⑥ MCP 三码业务层 matrix ⑦ `test_timeout_policy.py` 四层 timeout 跨层不变量 ⑧ background-task timeout E2E（:8101 低 MAX_TASK_DURATION 实例 → TASK_TIMEOUT + phase=error + FAILED 落库）⑨ DB timeout contract（真 PG 100ms override，kind=timeout 无假行）；repair case 同款收紧（用户拍板） | 分支 `p15-e2e-live`：`2191a51`(①②④+plan) → `558f30f`(③) → `0c3a8d1`(⑥⑦⑨) → `00aaa43`(⑧) → `268ea88`(⑤) → `a22c336`(repair)；offline contracts+smoke+persistence **751 passed** 零回归；关键发现（空洞 case / LLM 质量 gating / session API phase 字段）见 plan 落地记录 |
|---|---|---|
| [2026-09-02-p15-e2e-bugfix-and-formalize.md](2026-09-02-p15-e2e-bugfix-and-formalize.md) | P15 e2e bug 修复 + 正式用例固化：③ vector codec init_pool 统一注册（asyncpg 无 pgvector codec → `$1::vector` 绑 list 每次 DataError，recall 全断）→ ② supplement 全部已确认约束继承（prior_card 真合并 + prompt 承上轮槽）→ ① 产品修复（点名对象 ∉ schema → requested_object assumption 澄清不静默替换）+ env-gated fault seam（`X-E2E-Fault` header 双 gate，object_not_found→repair 确定性 / permission→fail-fast）→ ④ `evaluation/tests/test_real_rag_mcp_e2e.py` 重建带真断言 6 场景 | 11 commit（T1 codec `6633eef` → T2 supplement `2f4709a` → T3 `a751d57` → T4 seam `0ba6f67` → T5 `f6b0a89` → FK 同步 `5cae708` → 相对时间续接 `d76de95` → SQLAgentState channel `d61326e` → 断言收敛 `210c947` → docs）；离线 contracts+smoke+graphs **781 passed** / persistence vector recall 真 PG 绿 / evaluation 40 passed+6 skipped；**live 6 例逐一绿于最终代码**（backend REPORTAGENT_E2E=1；repair/fail seam 真触发）；env 修复 query_template 4096→1536；落地偏差 6 项见 plan 落地记录；下一步真实用例扩充（memory `user-e2e-expansion-directive`） |
| [2026-09-01-p13-langfuse.md](2026-09-01-p13-langfuse.md) | P13 实施：Langfuse 接入 + PII redaction——`backend/app/observability/` 新包（LangfuseConfig env-gated + client 单例 + redaction 递归 mask 复用 `app/utils/pii.py` mask_pii + flush 适配器）；Tracer 双 sink（PG `infra/trace/repository.py` + Langfuse SDK，env `LANGFUSE_PUBLIC_KEY`+`LANGFUSE_SECRET_KEY` 设时启用否则仅 PG）+ user_query / span input / output 全入口 PII mask；Prompt version + Model 落 Langfuse metadata；Langfuse SDK v4（OTel-based `start_as_current_observation` + `trace_context` 钉 trace_id 与 PG 关联） | 分支 `p13-langfuse`：`f17c4c6`(T1 LangfuseConfig) → `493645f`(T2 redaction) → `7eb5595`(T3 flush 适配器) → `98de42f`(T4 双 sink) → docs commit；后端 **979 passed / 1 skipped / 5 warnings**（956 baseline + 23 新例）；Playwright Contract **9/10 全绿**——spec 05（SQL 耗预算→error band）在 master 与 p13 一样失败（A/B 已证非 P13 回归，后端 FAILED 版本落库正常，前后端渲染时序问题另查）；真用 cloud key 实测抓到 2 个 P13 缺陷并补 commit（`0b6d5f6` UUID trace_id 转 32-hex 供 Langfuse v4 / `6ab131c` 6 个 prompt builder 接线 record_prompt_version）；落地偏差 4 项见 plan 落地记录；CLAUDE.md §12 现状已更新 |
| [2026-09-01-p13-langfuse-execution-timing.md](2026-09-01-p13-langfuse-execution-timing.md) | P13 补完：Span 真实执行耗时回放 Langfuse observation metadata——`flush_to_langfuse` 给 span observation 加 `execution_duration_ms` / `original_start_time` / `original_end_time`（与 PG `spans.duration_ms`/`start_time`/`end_time` 一一对账）；root observation 加 `total_duration_ms` / `original_start_time` / `original_end_time`；LLM `latency_ms` 不动（语义分层：Agent 节点总耗时 vs LLM 单独往返耗时）。不改 Tracer 计时、不改 OTel context manager 嵌套、不硬塞 `start_time=`/`end_time=` 参数（Langfuse v4 SDK 不暴露）；字段缺失（None）过滤不入 metadata 防污染 | 分支 `p13-langfuse`：`1a14946`（fix flush_to_langfuse 写 timing 三字段 + `_timing_fields()` helper + 4 个新 contract 例）+ docs commit；后端 **990 passed / 1 skipped / 5 warnings**（979 baseline + 4 新例 + 7 P13 review 后扩展，零回归）；observability contract 29/29 全绿；user 真 cloud key 实测反馈"节点耗时没同步到 Langfuse"；落地偏差 3 项见 plan 落地记录 |
...
| Plan | 主题 | 落地 |
|---|---|---|
| [2026-09-01-p14-evaluation-skeleton.md](2026-09-01-p14-evaluation-skeleton.md) | P14 评估骨架 + 行为期望：9 子包目录终态（requirement/memory/retrieval/tool_selection/sql/repair/report/frontend/e2e）+ dispatcher pattern（DIM_REGISTRY + LEGACY_KEYS frozenset + register_dim）+ build_dim_results 纯函数（11 slot 聚合）+ 每子包 test_harness_<dim>.py 行为期望测试（baseline_cases.json 保持 13 类冻结）+ frontend/e2e 子包仅占位 + evaluation/README.md 用法文档 | 分支 `p14-evaluation-skeleton`：10 commit（4 feat + 4 fix P0/P1/P2/P3 + 2 docs）+ merge `10793c8`；evaluation/ **108 passed**（97 P14a + 11 P0/P1/P2/P3 fix）；backend/ **990 passed / 1 skipped / 5 warnings**（与 P13 baseline 完全一致，零回归）；4 P0/P1/P2/P3 issue 闭环：TurnExpectation extra='allow' 穿透 Pydantic / runner._observe_turn max(version) / 9 子包 import 移到 evaluation/__init__.py / run_case 三路径 dim_results 同形；落地偏差 5 项见 plan 落地记录 |
| [2026-09-02-fix-sql-classification-and-e2e.md](2026-09-02-fix-sql-classification-and-e2e.md) | P15 prelude 双 issue 合并：fix issue（SQL object/schema error 错误分类触发无信息增益 retry）+ test issue（建立真 RAG MCP + PG 的 Analytics Case e2e 取代 P14 mock）——`_classify_psycopg2_error` 用 `psycopg2.errors` 精确子类（UndefinedColumn/Table/Function→object_not_found、AmbiguousColumn→object_ambiguous、DivisionByZero/DatatypeMismatch→other）；DiagnosePolicy 方案 A（object_not_found → retry_mcp_schema_retrieval 1 次 → escalate clarify；object_ambiguous → clarify；retry_counters 三 key 正交 sql_generation/plan/mcp_schema；MAX_MCP_RETRIES 独立 settings）+ DiagnoseDecision Literal 扩展（action / retry_target 各加一项）；SQL_ERROR_KINDS 6→8 kind；删除 P14 14 个 mock test 文件（7 evaluation/tests/ + 7 子包 tests/）+ 新建 `evaluation/tests/test_real_rag_mcp_e2e.py` env-gated 5 类最小集 | 分支 `p15-prelude-fix-and-test`：6 commit（`3d41c1e` Task 1 SQL 分类精确化 + `9985ac7` Task 4 删 mock + `f218068` Task 5 真 e2e 套件 + `9baff76` Task 3 DiagnosePolicy 方案 A + `608bd9f` Task 6 test_sql_limits 同步 + docs）→ merge 待 Step 6.6；后端 contracts+smoke **676 PASS / 0 FAIL / 3 warnings**；evaluation **40 PASS / 5 SKIPPED**（env-gated 默认 skip）；后端全量回归 **1008 PASS / 1 SKIPPED / 5 warnings**（990 baseline + 18 P15 增量，零回归）；5 类 e2e 真链路验证留用户手动门（PG + ragent-py MCP + backend 全启动 + `REPORTAGENT_E2E=1`）；落地偏差 5 项（SyntaxError 漏补 / test_reliability_errors 漏同步 / plan typo / Step 5.1 笔误 / test_sql_limits 漏同步）见 plan 落地记录 |
