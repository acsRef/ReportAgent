...
| Plan | 主题 | 落地 |
|---|---|---|
| [2026-09-01-p13-langfuse.md](2026-09-01-p13-langfuse.md) | P13 实施：Langfuse 接入 + PII redaction——`backend/app/observability/` 新包（LangfuseConfig env-gated + client 单例 + redaction 递归 mask 复用 `app/utils/pii.py` mask_pii + flush 适配器）；Tracer 双 sink（PG `infra/trace/repository.py` + Langfuse SDK，env `LANGFUSE_PUBLIC_KEY`+`LANGFUSE_SECRET_KEY` 设时启用否则仅 PG）+ user_query / span input / output 全入口 PII mask；Prompt version + Model 落 Langfuse metadata；Langfuse SDK v4（OTel-based `start_as_current_observation` + `trace_context` 钉 trace_id 与 PG 关联） | 分支 `p13-langfuse`：`f17c4c6`(T1 LangfuseConfig) → `493645f`(T2 redaction) → `7eb5595`(T3 flush 适配器) → `98de42f`(T4 双 sink) → docs commit；后端 **971 passed / 1 skipped / 5 warnings**（956 baseline + 15 新例）；Playwright Contract **9/10 全绿**——spec 05（SQL 耗预算→error band）在 master 与 p13 一样失败（A/B 已证非 P13 回归，后端 FAILED 版本落库正常，前后端渲染时序问题另查）；落地偏差 4 项（plan 的 `start_as_current_span` v2 API 已迁移到 v4 OTel API / `Tracer.__init__` 对 None 保持 None 防 A-3 backfill 短路 / P8 D5 decisions 一并入 Langfuse metadata / opentelemetry 降回 1.41.1 保 chromadb 共存）见 plan 落地记录；CLAUDE.md §12 现状已更新 |
