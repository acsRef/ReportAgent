...
| Plan | 主题 | 落地 |
|---|---|---|
| [2026-09-04-p16-memory-evaluation.md](2026-09-04-p16-memory-evaluation.md) | P16 Memory Evaluation & Behavioral Tests（Memory 封版）：report 链 ContextRuntime 偏好接线（§三 Report=Preference 契约补全，确定性 apply_chart_preference 通道，prompt/前端零改动）+ EXECUTION 档 drop preference + 8 行为测试（preference→chart ON/OFF 对照 / 不污染 SQL / query memory 参考帧 / candidate & expired & cross-user Agent 层）+ Layer 3 Gold Set（25 记忆 + 30 金标 queries 30/30×3 稳定 + 真 embedding runner Recall@1 0.84 / @3 0.96 / MRR 0.90 / Clean 1.0）+ 真 LLM SQL 手动门待用户 | 分支 `p16-memory-evaluation`：`5905998`(plan) → `719195c`(D4) → `2d94213`(preference 纯函数) → `81f5eec`(report 接线) → `073177d`(①⑧) → `106905e`(②⑤⑥⑦) → `532188d`(③④) → `f7e9ee5`(gold set) → `28b0c2d`(runner) → `50ce283`(回归适配+docs)；backend **1176 passed/1 skipped** + evaluation 78 passed 零回归；执行偏差 3 项 + G4 手动门待办见 plan 落地记录 |
