-- ═══════════════════════════════════════════════════════════════
-- ReportAgent 示例数据 — 零售+电商 星型模型
-- 覆盖销售、退货、库存三个业务域，6 个维度表 + 3 个事实表
-- ═══════════════════════════════════════════════════════════════

-- ── 1. 日期维度 (dim_date) ───────────────────────────────────
CREATE TABLE dim_date AS SELECT * FROM (VALUES
...
) AS t(date_id, full_date, year, quarter_num, quarter, week_of_year, day_name, is_holiday);

-- ── 2. 区域维度 (dim_region) ─────────────────────────────────
CREATE TABLE dim_region AS SELECT * FROM (VALUES
  (1, '华北', '北京', '北京', '一线'),
  (2, '华北', '天津', '天津', '一线'),
  (3, '华北', '河北', '石家庄', '二线'),
  (4, '华东', '上海', '上海', '一线'),
  ...
  (16, '东北', '辽宁', '沈阳', '二线'),
  (17, '东北', '黑龙江', '哈尔滨', '二线')
) AS t(region_id, region_name, province, city, tier);
...
) AS t(product_id, product_name, category, sub_category, brand, unit_price, cost_price, supplier);
...
) AS t(customer_id, customer_name, customer_tier, industry, city, register_date);

-- ── 5. 仓库维度 (dim_warehouse) ─────────────────────────────
CREATE TABLE dim_warehouse AS SELECT * FROM (VALUES
  (1, '北京中心仓', '北京', 50000),
  (2, '上海中心仓', '上海', 60000),
  (3, '广州中心仓', '广州', 45000),
  (4, '成都中心仓', '成都', 35000),
  (5, '西安分仓', '西安', 15000)
) AS t(warehouse_id, warehouse_name, city, capacity);

-- ── 6. 员工维度 (dim_employee) ──────────────────────────────
CREATE TABLE dim_employee AS SELECT * FROM (VALUES
...
) AS t(employee_id, employee_name, department, position, city, hire_date);
...
-- ── 7. 销售事实 (fact_sales) ─────────────────────────────────
-- 40 条销售记录，覆盖多个区域/产品/时间，含不同折扣和利润情况
...
) AS t(sale_id, date_id, product_id, region_id, customer_id, channel, quantity, unit_price, discount, total_amount, cost_amount, profit);
...
-- ── 8. 退货事实 (fact_returns) ───────────────────────────────
-- 12 条退货记录，让"退货率"分析有数据可查
CREATE TABLE fact_returns AS SELECT * FROM (VALUES
...
) AS t(return_id, sale_id, product_id, return_date_id, return_quantity, return_amount, return_reason, handling);

-- ── 9. 库存事实 (fact_inventory) ─────────────────────────────
-- 让"库存不足""库存周转"等查询有数据
CREATE TABLE fact_inventory AS SELECT * FROM (VALUES
...
) AS t(inventory_id, product_id, warehouse_id, date_id, quantity_on_hand, quantity_reserved, quantity_available);

-- ── 10. 考勤事实 (fact_attendance) ───────────────────────────
CREATE TABLE fact_attendance AS SELECT * FROM (VALUES
  (1, 1, 20240301, '正常', 8.0),
  (2, 2, 20240301, '正常', 8.0),
  (3, 3, 20240301, '正常', 8.0),
  (4, 4, 20240301, '正常', 8.0),
...
) AS t(attendance_id, employee_id, date_id, status, work_hours);