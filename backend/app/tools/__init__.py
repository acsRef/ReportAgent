...
def register_all_tools():
    registry.register(
        ...
        ToolMetadata(
            ...
            when_to_use="不知道数据在哪个表时找表，如 退货原因/销售额/库存 等业务概念需定位表",
            ...
            examples=["search_tables('退货原因分析') → fact_returns", "search_tables('各区域销售额', top_k=5)"],
        ),
    )
    registry.register(
        "get_table_ddl",
        ...
        ToolMetadata(
            ...
            examples=["get_table_ddl('dim_region') → 含 region_name/province/city", "get_table_ddl('fact_sales')"],
        ),
    )
    registry.register(
        "list_tables",
        ...
        ToolMetadata(
            ...
            side_effects="只读，无副作用",
            examples=["list_tables() → 含 fact_sales/dim_region 等 10 表"],
        ),
    )
    registry.register(
        ...
        ToolMetadata(
            ...
            side_effects="只读，无副作用",
            examples=["search_interface_dictionary('total_amount 是什么')", "search_interface_dictionary('GMV 口径')"],
        ),
    )
    registry.register(
        "search_faq",
        ...
        ToolMetadata(
            ...
            when_to_use="写 SQL 前查这类问题以前怎么算，如毛利率/退货率/出勤率/库存周转等口径与分组排序模板",
            ...
            side_effects="只读，无副作用",
            examples=["search_faq('毛利率怎么算')", "search_faq('区域退货率')"],
        ),
    )
    registry.register(
        "validate_sql",
        ...
        ToolMetadata(
            ...
            examples=["validate_sql('SELECT * FROM fact_sales') → valid=true", "validate_sql('DROP TABLE x') → valid=false"],
        ),
    )
    registry.register(
        "execute_sql",
        ...
        ToolMetadata(
            ...
            examples=["execute_sql('SELECT region, SUM(amount) FROM fact_sales GROUP BY 1')"],
        ),
    )
    registry.register(
        "chart_advisor",
