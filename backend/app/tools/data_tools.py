...
def search_tables(query: str, top_k: int = 3) -> str:
    ...
    用途：不知道数据在哪个表时用来「找表」。
    场景：用户提问含销售额/退款率/趋势等业务概念，需要先定位表。
    反例：已经知道表名（如 fact_orders），只是要看它的字段 → 用 get_table_ddl。
    输入：query（中文描述），top_k（返回条数，默认 3）
    输出：JSON 数组，每项 {table_name, columns[{name,type}], ddl, description, score}
    示例：search_tables('2024年华东各门店销售额') → 优先返回 fact_orders（订单事实表）
...
def get_table_ddl(table_name: str) -> str:
    ...
    输入：table_name（英文下划线格式，如 'fact_orders'、'dim_product'）
    输出：CREATE TABLE 文本
    示例：get_table_ddl('dim_store') → 返回含 store_id、region、city、store_type 的建表语句
