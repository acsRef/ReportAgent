...
> 现状（2026-09-04 P16 Memory Evaluation & Behavioral Tests 后，Memory 封版）：① **report 链补 ContextRuntime 偏好接线**（§三 Report = Semantic ✅ Preference 契约补全——P4c 只翻了 4 个 caller，report 是契约内遗漏）——`report_graph._plan_analysis` async 化接入 `ContextRuntime.build()`（agent=report_plan_analysis → REPORT 档，decision 层早备好），preference 类召回（source=memory_preference / kind=preference）→ `report_preferences`，`_build_output` 经**确定性纯函数** `app/report/preference.py apply_chart_preference` 应用（词表交集 {pie, bar, table}、软约束数据硬约束让位、dims 键归一；prompt / chart_advisor / registry / frontend 零改动）② **EXECUTION 档 drop preference**（runtime.py Step 4.5，§三 Execution=业务事实契约修正——图表偏好不再进 SQL 上下文）③ **行为测试 8 钉**（backend/tests/persistence/，真 PG + mock LLM + fake embedder）：① 偏好柱状图 override 默认 pie + ⑧ ON/OFF 对照 / ② preference 不进 SQL context（business fact 可留）/ ③④ query memory → SQL 上下文参考帧召回 + COUNT 参考仅作数据（防御帧）/ ⑤⑥⑦ candidate & expired & cross-user 在 Agent 层（ContextRuntime.build 产出）排除 ④ **Layer 3 Gold Set 召回评测**：`evaluation/memory_gold_cases.json`（25 记忆 + 30 金标 queries，词交确定性设计）+ `backend/tests/persistence/test_memory_gold_recall.py`（30/30 ×3 稳定：gold 必中 / forbidden 零词交或档位排除必不召 / expect_none = policy 保守空召回）+ 真 embedding runner `evaluation/gold_memory_runner.py`（真跑 ×2 稳定快照：**Recall@1 = 0.84 / Recall@3 = 0.96 / MRR = 0.90 / Clean = 1.0**，25 cases，快照在 evaluation/results/）。全量回归 backend **1176 passed / 1 skipped**（1116 baseline + ~25 P16 增量，test_tool_descriptions 一例随 async 节点适配）+ evaluation 78 passed。真 LLM SQL 行为手动门（plan G4）**已跑完（2026-09-07）**：`evaluation/tests/test_memory_llm_sql_behavior.py` env-gated 4 case（control/①偏好不污染/②参考复用+时间更新/③COUNT 不诱导），真 LLM+真 embedding 多轮采样②修复后 3/3（去年=2025 ✅）、①③ 各 3/3、control 4/5（2 次 LLM 彩票重跑收敛）；**发现并修复真缺陷**：参考帧年份 anchor 诱导时间偏移（无记忆去年=2025 正确 / 有 2024 参考帧时去年=2023）→ `format_context_block` 防御帧补「时间以 today 为准」→ 修复后 =2025 ✅，帧文案离线 39 passed 零回归。⑤ **P16.5（2026-09-07，用户拍板）**：简历盘点发现 §五 explicit→active 写入口**无生产 caller**——`_chat_requirement_analysis` 图前短路（`extract_explicit_preference` 正则 → `remember_explicit_preference` → idle 轻响应复用 chitchat 渲染，前端零改动；写入失败 best-effort 不阻塞），UI「以后都用柱状图」→ active/high 行 → report 档召回闭环；全量 **1181 passed / 1 skipped**，合 master `d1c10f4`。
...
> 现状（2026-09-01 P13 落地 + execution-timing 补完）：tracer 双 sink（PG `infra/trace/repository.py` + Langfuse SDK `app/observability/langfuse_flush.py`，env `LANGFUSE_PUBLIC_KEY`+`LANGFUSE_SECRET_KEY` 设时启用，否则仅 PG 路径）+ PII redaction 层（`app/observability/redaction.py` 递归 mask dict/list/str，复用 `app/utils/pii.py` mask_pii；user_query / span input / output 全部入口 mask）+ Prompt version + Model 字段落 Langfuse metadata + **业务真实耗时回放**（span observation `metadata.execution_duration_ms` / `original_start_time` / `original_end_time`；root `metadata.total_duration_ms` + `original_*`；LLM `metadata.latency_ms`；Langfuse native duration 不可信，metadata 是真值；与 PG `spans`/`traces` 一一对账）。LangChain CallbackHandler 未引入（adapter P6 锁定）；Langfuse SDK v4（OTel-based，`start_as_current_observation` + `trace_context` 钉 trace_id 与 PG 关联）。详见 plan `2026-09-01-p13-langfuse.md` 与 `2026-09-01-p13-langfuse-execution-timing.md`。
