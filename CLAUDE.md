...
```text
User ←SSE→ React + Vite (:3000) ←→ FastAPI + LangGraph Agent (:8100) ←MCP→ MCP Schema Server (:8101)
                                                                              |
                                                                         DuckDB (read-only)
                                                                              |
...
Two backend services and one frontend:

- **Frontend** (:3000) — React + Vite + Ant Design + ECharts, dual-panel layout
...
| Rendering | ECharts (SVG) + custom ReportBlock component system |
| State Management | Zustand (with localStorage session persistence) |
...
backend/
  app/
    ...
    agent/
      security_guard.py   — ⭐ Prompt injection guard (10 regex rules, score ≥ 3 → HIGH)
      parent_graph.py     — Parent LangGraph (8 nodes), only clarify node calls interrupt()
      data_graph.py       — Data SubGraph: schema discovery via MCP/local tools
      sql_graph.py        — SQL SubGraph: plan → generate → validate → execute → retry
      report_graph.py     — Report SubGraph: plan_analysis → run_step loop → finalize
    models/
      contracts.py        — 6 Pydantic models: SchemaContext, QueryPlan, QueryResult, ReportSpec, etc.
    tools/
      __init__.py         — 10 tools with Chinese decision boundary descriptions
      registry.py         — ToolRegistry with capability-based lookup + risk_level metadata
      data_tools.py       — Schema discovery tools (independent from MCP server's SchemaRegistry)
      sql_tools.py        — SQL validation/execution/AST parsing (3-layer safety)
      report_tools.py     — Chart advisor, trend analysis, group compare, anomaly detection
    infra/
      db/postgres.py      — asyncpg connection pool (min_size=2, max_size=10)
      trace/              — Tracer SDK + async PostgreSQL flush (sdk.py, repository.py, models.py)
      memory/             — MemoryManager + UserMemory + QueryMemory (pgvector hybrid ranking)
      checkpoint/         — Session management (factory.py, session.py)
    utils/
...
mcp_schema_server/
  server.py               — MCP server exposing search_tables, get_table_ddl, list_tables

frontend/
  src/
    api/                  — SSE client + chat API (parseSSEChunk, chatStream)
    stores/               — Zustand store (session.ts with localStorage persistence)
    adapter/              — reportAdapter.ts (normalizes backend ReportResponse → ReportBlock[])
    components/
      chat/               — QueryBar, AgentTimeline, EmptyState
      layout/             — AppShell, Navbar
      report/             — ReportRenderer + blocks/ (ChartBlock, TableBlock, InsightBlock, KpiBlock, MarkdownBlock)
    pages/                — ChatPage, HistoryPage, TemplateCenter
...
- **TSD-Encrypted source files:** Many `.py` files show `%TSD-Header-###%` and are unreadable in the working tree. Use `git show HEAD:<path>` to read decrypted content. Only `main.py`, `llm.py`, and empty `__init__.py` files are readable directly.
...
| `LLM_MODEL` | `MiniMax-M3` | Model name (also used: `MiniMax-M2.7-highspeed`) |
...
| `EMBEDDING_DIM` | `1536` | Vector dimension (must match `init_pg.sql`) |
...
# Frontend lint (oxlint — fast Rust-based linter)
...
- `docs/superpowers/plans/2026-07-15-report-mode.md` — Report mode feature plan.
